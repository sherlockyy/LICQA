...
class IQAPerformance(object):
    ...
    def compute(self, is_plot=False, fig_name='scatter_plot.png'):
        ...
        plcc, srcc, rmse = correlation_evaluation(pred, mos, is_plot=is_plot, plot_path=self.save_dir/fig_name)
...
def correlation_evaluation(obj_score, mos, is_plot, plot_path, xlabel='predicted score', ylabel='mean opinion score'):
    ...
    if is_plot:
        ...
        plt.title('scatter plot {} \n plcc: {:0.3f}, srcc: {:0.3f}, rmse: {:0.3f}'.format(fit_stat, plcc, srcc, rmse))
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
