...
class DISTS(torch.nn.Module):
    def __init__(self, pretrain='vgg16.pth', load_weights=False):
        super(DISTS, self).__init__()
        vgg = models.vgg16(pretrained=False)
        vgg.load_state_dict(torch.load(pretrain, {'map_location': lambda storage, loc: storage}))
    ...
    def forward(self, data, require_grad=False, batch_average=False):
        x, y = data
        if require_grad:
        ...
        if batch_average:
            return None, score.mean()
        else:
            return None, score


def make_model(args):
    return DISTS(pretrain=args.vgg16_pretrain)
